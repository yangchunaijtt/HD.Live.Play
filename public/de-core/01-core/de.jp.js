...
Date.prototype.MonthWeek = function (year, month, date) {
  /*
      month = 6 - w = 当前周的还有几天过完(不算今天)
      year + month 的和在除以7 就是当天是当前月份的第几周
  */
  var _date = this ? this : new Date()
  year = year || _date.getFullYear()
  month = month || _date.getMonth() + 1
  date = date || _date.getDate()
  var dateNow = new Date(year, parseInt(month) - 1, date)
