...
<script>
import classTmp from '@/components/biz/class/StList/index'
...
export default {
  ...
  data () {
    return {
      ...
      studentCaller: {},
      selectedDay: '今天',
      IDs: this.$store.getters['global/studentIDs'],
      timer: null,
      documentHeight: 0
    }
  },
  ...
  created: function () {
    this.studentCaller = new DE.Sch.SchStudent.Caller()

    console.log('student:' + DE.Instance.Phone)
    this.studentCaller.getStudentsByPhone(DE.Instance.Phone).then(function (value) {
  ...
  methods: {
    ...
    handlerRangeChange: function (range) {
      ...
      this.studentCaller.getLessonsByRange(begin, end, this.IDs).then(res => {
    ...
    fetchCourseList (date) {
      ...
      var sparam = arr.join('&')
      console.log(1111, sparam)
      window.location.hash = window.location.hash.split('?')[0] + '?' + sparam

      this.studentCaller.getLessonsByDay(
        date, this.IDs
      ).then(res => {
